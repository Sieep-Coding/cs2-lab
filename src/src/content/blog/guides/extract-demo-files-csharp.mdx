...
excerpt: "Building analytics pipelines for CS2 demos using DemoFile.Net—with real code examples that actually run."
...
**Null safety.** Entities are created and destroyed mid-demo. Always check for null:
...
**Tick timing.** Not all ticks fire all events. `TickEnd` fires for most ticks, but some events happen between ticks. If you need precise timing, cross-reference with the timestamp in the event itself.

**Memory in loops.** Allocating strings or lists inside `TickEnd` will cause GC pressure. Use object pools or `ValueTuple` where possible:
